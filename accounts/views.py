...
def personal_details(request, user_id):
    if not request.user.is_authenticated():
        ...
        return render(request, 'accounts/personal_details.html', context)
...
def delete_account(request, user_id):
    if not request.user.is_authenticated():
        ...
        user.delete()
        return render(request, 'accounts/regret.html')


def account_delete_send_email(toaddr):
    text = "Greetings!\nYou have successfully deleted your ChatApplication Account\nIf your account is deleted " \
           "without your knowledge please contact our Support Team or Administrator for help.\nThank you for being " \
    ...
    if not request.user.is_authenticated():
        ...
        if request.method == 'POST':
            if form.is_valid():
                ...
                return render(request, 'accounts/personal_details_update.html', context)
        else:
            context = {
                'form': form,
            }
